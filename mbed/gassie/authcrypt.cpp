...
int Authcrypt::run(string msg_to_encrypt)
{
    mbedtls_printf("\n\n");
    print_hex("plaintext message",
              reinterpret_cast<const unsigned char *>(msg_to_encrypt.c_str()),
              msg_to_encrypt.length());
    ...
    ret = mbedtls_cipher_auth_encrypt(&cipher, ciphertext, nonce_len,
                        ...
                        ciphertext + nonce_len + msg_to_encrypt.length(),
                        tag_len);
